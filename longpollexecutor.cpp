...
LongPollExecutor::LongPollExecutor(const Application* application, QObject* parent) :
    QObject(parent),
    ...
    ts("-")
{
    ...
    acquireLongPollServer();
}

void LongPollExecutor::acquireLongPollServer() {
    ...
    if (ts == "-") {
        ts = longPollConfigMap.value("ts").toString();
    }
}
